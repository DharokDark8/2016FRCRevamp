...
public class RevDisplay
{
	...
	final byte DECIMAL_POINT = 0b01000000;					//Bit to urn on a decimal point.
	...
	public RevDisplay () {
		...
		//charreg[39][0] = (byte)0b00000000; charreg[39][1] = (byte)0b00000000; //
	...
	public void revDisplayInit () {
		// set up the board - turn on, set blinking and brightness, This is required to turn on the display.
    ...
    byte getCharReg (String s, int i) {
    ...
    public void display(String s){
    	...
    	i2c.writeBulk(byte1);
    		...
    		this.scrollDisplay(s);
    	}

    } //public void Display(String s[])


    public void scrollDisplay(String s) {
		
    	s = s.toUpperCase();
    	byte[] byte1 = new byte[10];
      	int c0, c1, c2, c3;
      	int counter = 0;
      	String str = s.substring(counter, counter + 4);
      	scrollTimer.start();
      	if(scrollTimer.get() >= .5){
      		// first reset the array to zeros
          	for(int c = 0; c < 10; c++)
          	{
          			byte1[c] = (byte)(0b00000000) & 0xFF;
          	...
          	if (str.length() >= 1) c0 = getCharReg(s, 0); else c0 = SPACE_CHARACTER_INDEX;
          	if (str.length() >= 2) c1 = getCharReg(s, 1); else c1 = SPACE_CHARACTER_INDEX;
          	if (str.length() >= 3) c2 = getCharReg(s, 2); else c2 = SPACE_CHARACTER_INDEX;
          	if (str.length() >= 4) c3 = getCharReg(s, 3); else c3 = SPACE_CHARACTER_INDEX;
        	//if the string is too short put in space character instead.
